...
def get_metar_observations(timestep: dt.datetime) -> str:
    remote_path = BASEURL + f"/surface/{timestep.strftime('%Y%m')}/{timestep.strftime('%Y%m%d')}/Surface_METAR_{timestep.strftime('%Y%m%d')}_0000.nc"
    local_path = os.path.basename(remote_path)
    return _download_file(remote_path, local_path)


def get_buoy_observations(timestep):
    remote_path = BASEURL + f"/surface/{timestep.strftime('%Y%m')}/{timestep.strftime('%Y%m%d')}/Surface_Buoy_{timestep.strftime('%Y%m%d')}_0000.nc"
    local_path = os.path.basename(remote_path)
    return _download_file(remote_path, local_path)


def get_surface_synoptic_observations(timestep):
    remote_path = BASEURL + f"/surface/{timestep.strftime('%Y%m')}/{timestep.strftime('%Y%m%d')}/Surface_Synoptic_{timestep.strftime('%Y%m%d')}_0000.nc"
    local_path = os.path.basename(remote_path)
    return _download_file(remote_path, local_path)


def get_upper_air_observations(timestep):
    remote_path = BASEURL + f"/upperair/{timestep.strftime('%Y%m')}/{timestep.strftime('%Y%m%d')}/Upperair_{timestep.strftime('%Y%m%d')}_0000.nc"
    local_path = os.path.basename(remote_path)
    return _download_file(remote_path, local_path)
...
if __name__ == "__main__":
    # From 2009 to now
    date_range = pd.date_range(start="2009-01-01", end=dt.datetime.now().strftime("%Y-%m-%d"), freq="D")
