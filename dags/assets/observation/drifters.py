...
date_range = pd.date_range("2012-01-01", "2018-09-30", freq="1ME")
for date in date_range[::-1]:
    ...
    try:
        if os.path.exists(f"{base_path}{pre_thing}All/{date.strftime('%Y-%m')}_all.nc"):
            try:
                xr.open_dataset(
                    f"{base_path}{pre_thing}All/{date.strftime('%Y-%m')}_all.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}VolunteerShips/{date.strftime('%Y-%m')}_volunteership.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}VolunteerShips/{date.strftime('%Y-%m')}_volunteership.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
                ...
                with open(
                    f"{pre_thing}VolunteerShips/{date.strftime('%Y-%m')}_volunteership.nc", "wb"
                ) as f:
                    f.write(drifter_response.content)
        else:
            # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}Vosclim/{date.strftime('%Y-%m')}_vosclim.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Vosclim/{date.strftime('%Y-%m')}_vosclim.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}Station/{date.strftime('%Y-%m')}_station.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Station/{date.strftime('%Y-%m')}_station.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}Weather/{date.strftime('%Y-%m')}_weather.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Weather/{date.strftime('%Y-%m')}_weather.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}Glider/{date.strftime('%Y-%m')}_glider.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Glider/{date.strftime('%Y-%m')}_glider.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}Ships/{date.strftime('%Y-%m')}_ships.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Ships/{date.strftime('%Y-%m')}_ships.nc"
                ).load()  # If this succeeds, it was successful
            except:
        ...
        if os.path.exists(f"{pre_thing}Drifters/{date.strftime('%Y-%m')}_drifters.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Drifters/{date.strftime('%Y-%m')}_drifters.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}Moored/{date.strftime('%Y-%m')}_moored.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Moored/{date.strftime('%Y-%m')}_moored.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}Shore/{date.strftime('%Y-%m')}_shore.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Shore/{date.strftime('%Y-%m')}_shore.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
        ...
        if os.path.exists(f"{pre_thing}Tide/{date.strftime('%Y-%m')}_tide.nc"):
            try:
                xr.open_dataset(
                    f"{pre_thing}Tide/{date.strftime('%Y-%m')}_tide.nc"
                ).load()  # If this succeeds, it was successful
            except:
                # Do for drifters
