...
files = sorted(
    list(Path("/run/media/jacob/Elements/UK_Radar/").rglob("*.h5"))
)  # Get the others at same time
...
files = sorted(
    list(Path("/run/media/jacob/Elements/FMI_Radar/").rglob("*ACRR1H-3067-1KM.tif"))
)  # Get the others at same time
for i, file in tqdm.tqdm(enumerate(files), total=len(files)):
    try:
        data = open_and_clean_fmi_radar(file)
        data = data.merge(
            open_and_clean_fmi_radar(file.with_name(file.name.replace("ACRR1H", "ACRR24H")))
        )
        data = data.merge(
            open_and_clean_fmi_radar(file.with_name(file.name.replace("ACRR1H", "ACRR12H")))
        )
    except Exception as e:
        print(f"Failed to open {file}: {e}")
        continue
