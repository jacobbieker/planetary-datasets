...
def download_metoffice_global_init_time(
    it: dt.datetime, context: dg.AssetExecutionContext
) -> list[str]:
    ...
    for forecast_time in range(0, 55):
        for k, v in PRESSURE_VARIABLES.items():
            ...
            if file in available_files:
                fs.get(
                    file,
                    f"{ARCHIVE_FOLDER}{it.strftime('%Y%m%dT%H%MZ')}/{valid}-PT{str(forecast_time).zfill(4)}H00M-{k}.nc",
                )
...
@dg.asset(
    name="metoffice-global-download",
    description="Download MetOffice 10km Deterministic global model on AWS",
    tags={
...
def metoffice_dummy_zarr_asset(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    dummies_4d_var = dask.array.zeros(
        (len(date_range), len(data.step), data.latitude.shape[0], data.longitude.shape[0]),
        chunks=(1, 1, -1, -1),
        dtype=np.float16,
    )
    dummies_5d_var = dask.array.zeros(
        (
            ...
            len(data.step),
            len(data.pressure),
            data.latitude.shape[0],
            data.longitude.shape[0],
        ),
        chunks=(1, 1, -1, -1, -1),
        dtype=np.float16,
    )
    ...
    default_pressure_dataarray = xr.DataArray(
        dummies_5d_var,
        coords={
            "init_time": date_range,
            "step": data.step.values,
    ...
    dummy_dataset = xr.Dataset(
        {v: default_pressure_dataarray for v in data.data_vars if "pressure" in data[v].coords},
        coords={
            "init_time": date_range,
            "step": data.step.values,
            "pressure": data.pressure.values,
    ...
    dummy_dataset.update(
        {v: default_dataarray for v in data.data_vars if "pressure" not in data[v].coords}
    )
    dummy_dataset.chunk(
        {"init_time": 1, "step": 1, "pressure": -1, "latitude": -1, "longitude": -1}
    ).to_zarr(
        ZARR_PATH,
        mode="w",
        compute=False,
...
def combine_metoffice_to_dataset(downloaded_files: str) -> tuple[xr.Dataset, dict]:
    ...
    # Chunk it
    ds = ds.chunk(
        {"init_time": 1, "step": 1, "latitude": -1, "longitude": -1, "pressure": -1}
    ).sortby("step")
    return ds, encoding
