...
@dg.asset(
    name="imerg-late-download",
    description="Download IMERG early satellite precipitation data from NASA",
    tags={
...
def imerg_late_dummy_zarr(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    dummies = dask.array.zeros(
        (len(date_range), data.latitude.shape[0], data.longitude.shape[0]),
        chunks=(1, -1, -1),
        dtype=np.float16,
    )
...
def imerg_late_zarr_asset(
    ...
    for f in files:
        data = open_h5(f)
        data.chunk({"time": 1, "longitude": -1, "latitude": -1}).to_zarr(
            ZARR_PATH, region={"time": "auto", "latitude": "auto", "longitude": "auto"}
        )
        os.remove(f)
...
@dg.asset(
    name="imerg-final-upload-source-coop",
...
def get_imerg_late_files(day: dt.datetime) -> list[str]:
    ...
    return sorted(
        list(
            glob.glob(
                f"{ARCHIVE_FOLDER}gpm1.gesdisc.eosdis.nasa.gov/data/GPM_L3/GPM_3IMERGHH.07/{day.strftime('%Y')}/{str(day_of_year).zfill(3)}/*.HDF5"
            )
        )
    )
