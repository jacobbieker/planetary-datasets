...
@dg.asset(
    name="gmgsi-download",
    description="Download GMGSI global mosaic of geostationary satellites from NOAA on AWS",
    tags={
...
def gmgsi_v3_download_asset(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    for channel in ["VIS", "WV"]:
        ...
        s3_uri = (
            ...
            + list(
                fs.glob(
                    f"{BASE_URL}GMGSI_{channel}/{it.year}/{it.month:02}/{it.day:02}/{it.hour:02}/GLOBCOMP{channel}_v3r0_blend_s{it.strftime('%Y%m%d%H')}*"
                )
            )[0]
        )
    ...
    for channel, name in [("LW", "LIR"), ("SW", "SIR")]:
        ...
        s3_uri = (
            ...
            + list(
                fs.glob(
                    f"{BASE_URL}GMGSI_{channel}/{it.year}/{it.month:02}/{it.day:02}/{it.hour:02}/GLOBCOMP{name}_v3r0_blend_s{it.strftime('%Y%m%d%H')}*"
                )
            )[0]
        )
        local_uri = s3_uri.replace(BASE_URL, ARCHIVE_FOLDER)
...
def gmgsi_dummy_zarr_asset(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    dummies = dask.array.zeros(
        (len(date_range), data.xc.shape[0], data.yc.shape[0]), chunks=(1, -1, -1), dtype=np.uint8
    )
    default_dataarray = xr.DataArray(
        dummies,
        coords={
            "time": date_range,
            "latitude": (["yc", "xc"], data.latitude.values),
...
def gmgsi_zarr_asset(
    ...
    dataset.chunk({"time": 1, "yc": -1, "xc": -1}).rename(
        {"lat": "latitude", "lon": "longitude"}
    ).to_zarr(
        ZARR_PATH,
        region={"time": "auto", "xc": "auto", "yc": "auto"},
    )
    return dg.MaterializeResult(
        metadata={"zarr_path": ZARR_PATH},
    )
