...
@dg.asset(
    name="ghe-download",
    description="Download GHE global mosaic of geostationary satellites from NOAA on AWS",
    tags={
...
@dg.asset(
    name="ghe-download-navigation",
    description="Download GHE global mosaic of geostationary satellites navigation file from NOAA on AWS",
    tags={
        "dagster/max_runtime": str(60 * 60 * 10),  # Should take 6 ish hours
        "dagster/priority": "1",
        "dagster/concurrency_key": "download",
...
def ghe_download_navigation(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    if not os.path.exists(f"{ARCHIVE_FOLDER}NPR.GEO.GHE.v1.Navigation.netcdf"):
        fs.get(
            f"{BASE_URL}NPR.GEO.GHE.v1.Navigation.netcdf",
            f"{ARCHIVE_FOLDER}NPR.GEO.GHE.v1.Navigation.netcdf",
        )
    # Return the paths as a materialization
    return dg.MaterializeResult(
        metadata={
...
def gmgsi_zarr_asset(
    ...
    dataset.chunk({"time": 1, "lines": -1, "elems": -1}).to_zarr(
        ZARR_PATH,
...
def get_ghe_mosaic(time: dt.datetime) -> xr.Dataset:
    ...
    for minute_offset in range(0, 60, 15):
        ...
        ds = xr.open_dataset(
            f"{ARCHIVE_FOLDER}rain_rate/{time.year}/{time.month:02}/{time.day:02}/NPR.GEO.GHE.v1.S{time.strftime('%Y%m%d%H%M')}.nc.gz"
        ).load()
        # rename data to vis
